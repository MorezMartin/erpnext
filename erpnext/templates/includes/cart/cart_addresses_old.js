frappe.ready(() => {
	...
	$('#input_same_billing').change((e) => {
		...
		const address_name = $('[data-fieldname="shipping_address_name"]').find('.active').closest('[data-address-name]').attr('data-address-name')
		const address_type = $('[data-fieldname="shipping_address_name"]').find('.active').closest('[data-address-type]').attr('data-address-type')
		if ($check.is(':checked')) {
	...
	function setup_state() {
		...
		const cc = $('[data-shipping-rule-name][data-active]').closest('[data-shipping-rule-name]').is('[data-click-n-collect]')
