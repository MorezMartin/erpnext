...
def get_events(start, end, filters=None):
	...
	return frappe.db.sql(
		"""select `tabTimesheet Detail`.name as name,
			...
			CONCAT_WS(' ', ROUND(`tabTimesheet Detail`.hours, 2), 'h') as hrs,
			CONCAT_WS('\n', `tabTimesheet`.employee_name, `tabTimesheet Detail`.location_name, sales_order, activity_type, hrs) as title
